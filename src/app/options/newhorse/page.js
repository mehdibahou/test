...
export default function NewHorseForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    discipline: "",
    otherDiscipline: "",
    ecasSubCategory: "", // ECAS subcategory
  ...
  const handleFatherSearch = async (e) => {
    ...
    setFormData((prev) => ({
      ...
      father: null, // Reset father ID when text changes
    }));
  ...
  const handleMotherSearch = async (e) => {
    ...
    setFormData((prev) => ({
      ...
      mother: null, // Reset mother ID when text changes
    }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Concatenate ECAS with subcategory if applicable
      if (formData.discipline === "E.C.A.S") {
        if (
          ...
          finalDiscipline = `E.C.A.S-${formData.otherEcasSubCategory}`;
        } else if (formData.ecasSubCategory) {
          // Use the selected subcategory
          finalDiscipline = `E.C.A.S-${formData.ecasSubCategory}`;
        }
      }

      const submissionData = {
        ...formData,
        race: showOtherRace ? formData.otherRace : formData.race,
        robe: showOtherRobe ? formData.otherRobe : formData.robe,
        discipline: showOtherDiscipline
          ? formData.otherDiscipline
          : finalDiscipline,
        father: formData.father || formData.fatherText,
        mother: formData.mother || formData.motherText,
        Provenance: showOtherProvenance
          ? formData.otherProvenance
          : formData.Provenance,
        Detachement: showOtherDetachement
          ? formData.otherDetachement
          : formData.Detachement,
      };

      const BASE_URL = "http://localhost:3001/api";
      const response = await fetch(`${BASE_URL}/horse`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.success) {
        window.location.href = `/options/horses/horse?h=${data.data._id}`;
      } else {
        throw new Error(data.error || "Failed to create horse");
      }
    } catch (err) {
      ...
      <div className="mx-10 px-6 py-12">
        ...
        <div className="mb-10 relative">
          ...
          <p className="text-gray-600 mt-2">
            Enregistrer un nouveau cheval dans le système
          </p>
        </div>
