...
//     const pere = req.query.pere || "";
//     const mere = req.query.mere || "";
...
//     // Get horses with basic filters
//     let horses = await Horse.find(query)
//       .select(
...
//     // Apply post-query filtering for parent fields (père/mere)
//     if (pere || mere) {
//       horses = horses.filter((horse) => {
//         // Check father match
//         if (pere) {
//           const fatherMatch =
//             (horse.father && horse.father.name === pere) ||
//             horse.fatherText === pere;
...
//         if (mere) {
//           const motherMatch =
//             (horse.mother && horse.mother.name === mere) ||
//             horse.motherText === mere;

//           if (!motherMatch) return false;
//         }
...
exports.getHorses = async (req, res) => {
  try {
    ...
    const provenance = req.query.provenance || "";
    const detachement = req.query.detachement || "";
    // Add a new parameter to include or exclude radiated horses
    const includeRadiated = req.query.includeRadiated === "true";
    const radiationStatus = req.query.radiationStatus || "";
    console.log("Radiation status:", radiationStatus);

    let query = {};

    // Handle radiation status filtering
    if (radiationStatus === "radiated") {
      query.isRadie = true;
    } else if (radiationStatus === "active" || !includeRadiated) {
      query.isRadie = { $ne: true };
    }
    ...
    let horses = await Horse.find(query)
      .select(
        "name matricule race sex discipline robe birthDate etat father fatherText mother motherText horseId updatedAt Provenance Taille Puce Detachement isRadie motifderadiation dateRadiation isMutated"
      )
      .populate("father", "name matricule")
      .populate("mother", "name matricule");
    ...
    // Add radiation motifs
    const radiationMotifs = ["Mort", "Euthanasie", "Cession", "Vente", "Autre"];
    ...
    res.status(200).json({
      ...
      metadata: {
        ...
        detachements: filteredDetachements,
        etats: ["sain", "malade", "en rétablissement"],
        radiationMotifs: radiationMotifs,
      },
    });
  } catch (error) {
    console.error("Error fetching horses:", error);
